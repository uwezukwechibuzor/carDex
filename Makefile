...
HTTPS_GIT := https://github.com/uwezukwechibuzor/carDex.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=cardex \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=cardexd \
		  -X github.com/cosmos/cosmos-sdk/version.Version=$(VERSION) \
		  -X github.com/cosmos/cosmos-sdk/version.Commit=$(COMMIT) \
		  -X "github.com/cosmos/cosmos-sdk/version.BuildTags=$(build_tags_comma_sep)" \
		  -X "github.com/uwezukwechibuzor/carDex/app.EnableSpecificProposals=$(ENABLED_PROPOSALS)"
...
ifneq ($(GO_SYSTEM_VERSION), $(REQUIRE_GO_VERSION))
	@echo "ERROR: Go version ${REQUIRE_GO_VERSION} is required for $(VERSION) of cardex."
	exit 1
endif
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/cardexd ./cmd/cardexd
endif
...
build-static-linux-amd64: go.sum $(BUILDDIR)/
	...
	$(DOCKER) create -ti --name cardexbinary cardex-amd64
	$(DOCKER) cp cardexbinary:/bin/cardexd $(BUILDDIR)/cardexd-linux-amd64
	$(DOCKER) rm -f cardexbinary

install: check_version go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/cardexd

install-test-binary: check_version go.sum
	go install -mod=readonly $(BUILD_FLAGS_TEST_BINARY) ./cmd/cardexd
...
draw-deps:
	...
	@goviz -i ./cmd/cardexd -d 2 | dot -Tpng -o dependency-graph.png
...
kill-dev:
	@echo "Killing cardexd and removing previous data"
	-@rm -rf ./data
	-@killall cardexd 2>/dev/null
	-@killall gaiad 2>/dev/null

build-docker-image:

	@docker buildx build --load --build-context app=. -t cardex-node --build-arg BINARY=cardexd .

start-docker-container:

	@docker run --rm --name cardex -d -p 1317:1317 -p 26657:26657 -p 26656:26656 -p 16657:16657 -p 8090:9090 -e RUN_BACKGROUND=0 cardex-node
